## 2026-03-06 | Session 30: Round Stream Viewer One-Line Run + Verification

### What was added

- One-line launcher at repo root:
  - `run_viewer.ps1`
- Streaming replay viewer:
  - `scripts/round_stream_viewer.py`
- PowerShell launcher with transcript auto-pick:
  - `scripts/run_round_stream_viewer.ps1`
- Investigation/roadmap doc:
  - `docs/ROUND_STREAM_INVESTIGATION_PLAN.md`

### Precise run commands (PowerShell, from repo root)
...
```powershell
.\run_viewer.ps1 -RoundId 2
powershell -ExecutionPolicy Bypass -File scripts/run_round_stream_viewer.ps1 -RoundId 1
powershell -ExecutionPolicy Bypass -File scripts/run_round_stream_viewer.ps1 -TranscriptPath logs/archive/tournament_results_transcript.json -RoundId 1 -Speed 1.5
```
...
- Launched with `run_viewer.ps1`.
- Confirmed listener on `127.0.0.1:8787`.
- Verified SSE endpoint emits `data:` events including `round_start`.

---
## 2026-03-06 | Session 29: First Clean Campaign

### clean_overnight_01/clean_100 ó 99/100 runs, fixed economy, external judge

**Problems fixed this session:**
- Economy: pot raised 40?200 tokens (arguing now EV-positive above 50% confidence)
- Judge: switched from self-judge (qwen3-32b) to external (llama-3.3-70b-versatile)
- Prompt: deliberation v2 ó cost ratio (1 balance = 20 LLM tokens) and deliberation cost now visible to model
- Model name bug: Groq uses `llama-3.3-70b-versatile` not `meta-llama/llama-3.3-70b-versatile`

**Results:**

| Metric | Old (broken) | New (clean) |
|---|---|---|
| Pass rate | 0.83 | 0.491 |
| Alpha win rate | 70-80% | 54% |
| Health score | ó | 0.700 |
| Adaptation gain | ó | 0.033 |
| Total traces | contaminated | 2,954 clean |

**Key finding:** Models were always rational. They held because holding was correct EV under broken economy. Fix the incentives, behavior changes immediately.

**Data:** `logs/clean_overnight_01/clean_100/`
**Insights:** `docs/clean100_insights.md`
**Visualization:** `report_output/clean100_report.png`
**Config:** `configs/clean_economy_groq.yaml`

---

## 2026-03-05 | Session 28: Full Data Inventory

### All Campaign Data Located and Catalogued

Complete inventory of every test run with file paths, sorted by importance.

---

#### [1] Evolution Campaigns ó Directive Leaderboards
4 real Ollama 7b runs (10 stub runs with fitness=-0.267 are noise, ignore):

| Timestamp | Path | Key Result |
|---|---|---|
| 20260302T163806Z | `logs/evolution_campaign_20260302T163806Z/` | PUNISH_WEAKNESS 0.634 > Baseline 0.082 |
| 20260302T164759Z | `logs/evolution_campaign_20260302T164759Z/` | ANTI_RAMBLE 0.735 > PUNISH 0.712 > Baseline 0.659 |
| **20260302T165752Z** | `logs/evolution_campaign_20260302T165752Z/` | **ADAPT_ON_LOSS 0.680 > ANTI_RAMBLE 0.677 > Baseline 0.645 > PUNISH 0.350** ? main result |
| overnight/4_evo | `logs/overnight_20260305T022154Z/4_evolution/evolution_campaign_20260305T134310Z/` | PUNISH 0.672 > ANTI_RAMBLE 0.663 > Baseline 0.516 > ADAPT 0.418 |

---

#### [2] Overnight Experiments ó 4-Condition Comparison
`logs/overnight_20260305T022154Z/` ó 28+ tournaments, 4 conditions, fully successful

| Lane | Condition | Tournaments | Avg Pass Rate |
|---|---|---|---|
| `1_rookie/` | No playbook, fresh start | 10 | 0.19 |
| `2_veteran/` | Pre-loaded playbook | 10 | 0.83 |
| `3_direct/` | "Be smart & concise" prompt | 8 | 0.53 |
| `4_evolution/` | Directive evolution run | 24 | (evo format, no pass_rate) |

---

#### [3] Groq Multikey ó Bulk API Runs
`logs/groq_multikey_*/` ó 15 batch dirs, **65 successful tournaments** (604 failed, rate limits)
- Avg pass_rate: 0.560
- Data: `logs/groq_multikey_20260304T*/account_*/*/batch_summary.jsonl`

#### [4] Nvidia Afternoon ó Post-Prompt-Fix Local GPU
`logs/nvidia_afternoon/` ó **17 tournaments**, Ollama qwen2.5:7b, avg pass_rate: 0.571
(Healthy after RESPOND/HOLD prompt fix ó previously pass_rate was 1.0 due to invalid decision values)

#### [5] Stable Campaign 125 ó Emergent Baseline
`logs/stable_campaign_125/` ó 4/18 successful, pass_rate: 0.516

#### [6] Groq Campaign ó Standard Cloud
`logs/groq_campaign/` ó 5 tournaments, pass_rate: 0.568

#### [7] Veteran vs Newcomer ó Single Test
`logs/veteran_vs_newcomer/veteran_test.json`
- Beta (newcomer) won: 138 vs Alpha (veteran) 87 tokens
- A (veteran) learning improvement: -0.00061 (regressed), B: +0.000082
- **Inconclusive** ó 1 run only, same base model

---

#### [8] Archive ó Historical
`logs/archive/` ó notable subdirs:
- `emergent_campaign_250/` ó 113 files (JSONL traces, older long run)
- `groq_campaign_50/` ó 53 files (older 50-run groq batch)
- `skill_scout_tmp/` ó 646 files (auto-generated probes, low priority)
- rest: test scaffolding smoke tests, ignore

#### [9] Misc Partial / Failed-to-Launch Campaigns
- `logs/arc_afternoon/` ó 14 files, Arc GPU attempt (LM Studio not running)
- `logs/morning_queue_20260305_094201/` ó 27 files, queued batch
- `logs/local_nvidia_campaign/` ó 33 files
- `logs/groq_lean_afternoon/`, `logs/arc_groundtruth*/`, `logs/mixed_model_afternoon/` ó 1-6 files each, empty/aborted

---

## 2026-03-05 | Session 26: Exponential Economics & Agent Prompt Awareness

### What Was Implemented

- **Exponential Payout Curve**: Replaced the linear token split logic in `src/economy/distribution.py` with an exponential payout formula (using `exponent=2.0`). This aggressively skews payouts to the winner (e.g., 60/40 confidence now yields 42/18 tokens instead of 36/24 from a 60-token pot).
- **Agent Awareness Prompt**: Updated the `decide_bet` signature in `src/models/debater.py` to accept the `tokens_per_round` and exponent parameters.
- Injected an explicit economic awareness string into the deliberation prompt's `STATUS:` block. It dynamically defines the pot size, the exact exponential formula (`Payout = pot ◊ (score^2 / total_score^2)`), and provides a calculated 60/40 example. This gives the agents the exact variables needed to compute EV in their `<thinking>` phase and combats the "pacifism meta".

### Known Risks / Follow-ups

### Known Risks / Follow-ups

- The Groq Qwen3-32B agents are currently evaluating this in a 90-round multi-key deployment. We need to analyze those transcripts to confirm if the higher stakes induce strategic bankruptcies or aggressive play as predicted.

---

## 2026-03-05 | Session 27: Qwen3 Hybrid Reasoning & Groq Rate Limits

### What Was Diagnosed and Fixed

The system experienced intermittent `[VALIDATION_FAILED]` errors during Groq Qwen3-32B evaluations.

- **The "Alternating" Bug**: Qwen3 is a hybrid reasoning model. Without strict parameters, it auto-decides whether to output `<think>` tags on a per-request basis. This caused parsing to fail exactly 50% of the time when it chose to skip thinking (returning an empty string instead of the expected JSON wrapper).
- **The Fix**: Injected `reasoning_effort: "none"` exclusively for Groq backends in `openai_compat_backend.py`. This perfectly locked the model into deterministic, fast JSON responses and eliminated the alternating failures.

### Concurrency Tuning (Paid Tier)

- Scaled `.scripts/launch_groq_multikey.ps1` to harness the paid tier limits.
- **Hypothesis**: Paid tier could handle high concurrency (e.g. 10).
- **Result**: Concurrency=10 generated massive 40,000 TPM bursts and hit Groq's instantaneous Token-Per-Minute limit. The API returned 429 HTML pages which showed up as `"success": false` JSON-parsing errors in the batch summary.
- **Resolution**: Lowered the launch script to `Concurrency = 3`. The tournament now runs at maximum uninterrupted capacity (~12k TPM) with 100% success rate and zero validation errors.

---

## 2026-03-05 | Session 25: Experimental Multi-Agent Injection Wiring + Verification Pass

- Added experimental debater knobs in config loader:
  - `multi_agent_enabled`
  - `multi_agent_mode` (`none|critique|synthesize`)
  - `multi_agent_max_tokens`
  - `multi_agent_min_balance`
- Wired those knobs through tournament entrypoints:
  - `demo_tournament.py`
  - `demo_dynamic.py`
  - `src/benchmark/runner.py` seed tournament builder
- Implemented auto peer-draft generation in `src/models/debater.py`:
  - new `_build_peer_draft(...)` helper
  - auto-activation in `generate_argument(...)` when enabled and in counter mode
  - existing injection prompt path (`critique`/`synthesize`) now runs end-to-end
  - peer-draft token use is included in generation token accounting (`llm_tokens_used`)
- Added runnable preset:
...
- Added tests:
  ...
  - updated `tests/test_benchmark_config_parse.py`
  - updated `tests/test_suite.py`

### Verification Attempt + Outcome

- Static compile checks passed for all modified Python files (`python -m py_compile ...`).
- Runtime/test execution was blocked by missing dependency `pyyaml` in local environment.
- Install attempt failed due network/socket restrictions in this environment, so full runtime validation could not be completed here.

### Known Risks / Follow-ups

- Environment prerequisite remains required before runtime verification:
  - `pip install -r requirements.txt` (specifically `pyyaml>=6.0`).
- Experimental mode currently injects a peer draft generated by the same debater backend (role-split design). This is intentional for low-risk rollout; future upgrade can use heterogeneous advisor models.
...
_Chronological log of development progress_
...
- **DEC-042**: **OpenAI-Compatibility over Custom API** ó Opted for the OpenAI REST standard for vLLM to ensure future-proofing against other provider APIs (Anthropic, DeepSeek).
- **DEC-043**: **Consolidated Generation** ó Decided to enforce a single `_generate` entry point in models to ensure thinking extraction and cost tracking are applied consistently regardless of the backend.
...
1. **Blind Betting**: Debaters place a bet _before_ generating an argument, meaning they bet without knowing their generation costs or argument quality.
...
_Sessions 1-7 have been archived for context management. See [DEVLOG_ARCHIVE.md](./DEVLOG_ARCHIVE.md) for historical sessions including:_
